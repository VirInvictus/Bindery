...
def fix_title(content: str) -> str:
    ...
    content = re.sub(r'<title[^>]*/>', r'<title>Unknown</title>', content, flags=re.IGNORECASE)
    content = re.sub(r'<title[^>]*>\s*</title>', r'<title>Unknown</title>', content, flags=re.IGNORECASE)
    
    # Now check if it's completely missing
    head_m = re.search(r'<head[^>]*>(.*?)</head>', content, re.IGNORECASE | re.DOTALL)
    if head_m:
        if not re.search(r'<title[^>]*>.*?</title>', head_m.group(1), re.IGNORECASE | re.DOTALL):
            content = re.sub(r'(</head>)', r'<title>Unknown</title>\n\1', content, flags=re.IGNORECASE)
    return content
...
def test_fix():
    ...
    if modified:
        ...
        if "Element \"title\" must not be empty" not in res.stderr and "The \"head\" element should have a \"title\"" not in res.stderr:
